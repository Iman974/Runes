...
public class Unistroke {
    ...
    List<Vector2> ResampleToEvenlySpaced() {
        ...
        for (int i = 1; i < sourcePoints.Count; i++) {
            ...
            if ((D + distanceSinceLastEvenPoint) >= spacing) {
                ...
                Vector2 resizedVector = new Vector2 {
                    x = point1.x + ((spacing - D) * (point2.x - point1.x) / distanceSinceLastEvenPoint),
                    y = point1.y + ((spacing - D) * (point2.y - point1.y) / distanceSinceLastEvenPoint)
                };
